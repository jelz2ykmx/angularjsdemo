    ...
    public partial class Import : Form
    {
        ...
        async Task GetIds()
        {
            ...
            using (SqlConnection connection1 = new SqlConnection(connetionString))
            {
                ...
                String strcommand = "SELECT startdate, nextfitness, nextroutine, nextnutrition, nextsleep, nextweight, nextdiabetes, nextbiometrics " +
                                    "FROM ValidicUpdate where sync = 1";
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    {
                        ids.lastupdate = reader.GetDateTime(0);
                        ids.notNullDate = true;
                    }
                    if (!reader.IsDBNull(1))
                    {
                        ids.nextfitness = reader.GetString(1);
                    }
                    if (!reader.IsDBNull(2))
                    {
                        ids.nextroutine = reader.GetString(2);
                    }
                    if (!reader.IsDBNull(3))
                    {
                        ids.nextnutrition = reader.GetString(3);
                    }
                    if (!reader.IsDBNull(4))
                    {
                        ids.nextsleep = reader.GetString(4);
                    }
                    if (!reader.IsDBNull(5))
                    {
                        ids.nextweight = reader.GetString(5);
                    }
                    if (!reader.IsDBNull(6))
                    {
                        ids.nextdiabetes = reader.GetString(6);
                    }
                    if (!reader.IsDBNull(7))
                    {
                        ids.nextbiometrics = reader.GetString(7);
                    }
                }
                reader.Close();

                if (ids.nextfitness == "L" && ids.nextroutine == "L" && ids.nextnutrition == "L" && ids.nextsleep == "L"
                    && ids.nextweight == "L" && ids.nextdiabetes == "L" && ids.nextbiometrics == "L")
                {
                    ids.allDone = true;
                    ids.nextfitness = "";
                    ids.nextroutine = "";
                ...
                {
                    if (ids.nextfitness == "L")
                    {
                        ids.allDone = true;
                    }
                    if (ids.nextroutine == "L")
                    {
                        ids.allDone = true;
                    }
                    if (ids.nextnutrition == "L")
                    {
                        ids.allDone = true;
                    }
                    if (ids.nextsleep == "L")
        ...
        async Task StartUpdate()
        {
            ...
            if (ids.allDone)
            {
                startDate = ids.lastupdate.AddSeconds(1);
            }
            if (ids.nextfitness != "L")
            {
                await GetValidicData(0, ids, startDate, nowDate, "");
            }
            if (ids.nextroutine != "L")
            {
                await GetValidicData(1, ids, startDate, nowDate, "");
            }
            if (ids.nextnutrition != "L")
            {
                await GetValidicData(2, ids, startDate, nowDate, "");
            }
            if (ids.nextsleep != "L")
            {
                await GetValidicData(3, ids, startDate, nowDate, "");
            }
            if (ids.nextweight != "L")
            {
                await GetValidicData(4, ids, startDate, nowDate, "");
            }
            if (ids.nextdiabetes != "L")
            {
                await GetValidicData(5, ids, startDate, nowDate, "");
            }
            if (ids.nextbiometrics != "L")
            {
                await GetValidicData(6, ids, startDate, nowDate, "");
            }
        
        }

        async Task GetValidicData(int validicObject, Ids id, DateTime startDate, DateTime nowDate, String next)
        {
            ...
            else
            {
                if (task.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    if (token.ToString() == "200")
                    {
                        ...
                        await insertData(o, validicObject, ids, startDate, nowDate, next);

                        if (next != "")
                        {
                            await GetValidicData(validicObject, ids, startDate, nowDate, next);
                        }
                    }
                    else
                    {
        ...
        async Task insertData(JToken o, int validicObject, Ids ids, DateTime startDate, DateTime nowDate, String next)
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                using (SqlConnection connection1 = new SqlConnection(connetionString))
                {
                    ...
                    for (int x = 0; x < count; x++)
                    {
                        ...
                        if (validicObject == 0)
                        {
                            int updates = await UpdateFitness(connection1, o, validicObject, x, idActivity, idSource);
                            if (updates == 0)
                            {
                                await SaveFitness(connection1, o, validicObject, x, idActivity, idSource);
                            }
                        }
                        else if (validicObject == 1)
                        {
                            int updates = await UpdateRoutine(connection1, o, validicObject, x, idActivity, idSource);
                            if (updates == 0)
                            {
                                await SaveRoutine(connection1, o, validicObject, x, idActivity, idSource);
                            }
                        }
                        else if (validicObject == 2)
                        {
                            int updates = await UpdateNutrition(connection1, o, validicObject, x, idActivity, idSource);
                            if (updates == 0)
                            {
                                await SaveNutrition(connection1, o, validicObject, x, idActivity, idSource);
                            }
                        }
                        else if (validicObject == 3)
                        {
                            int updates = await UpdateSleep(connection1, o, validicObject, x, idActivity, idSource);
                            if (updates == 0)
                            {
                                await SaveSleep(connection1, o, validicObject, x, idActivity, idSource);
                            }
                        }
                        else if (validicObject == 4)
                        {
                            int updates = await UpdateWeight(connection1, o, validicObject, x, idActivity, idSource);
                            if (updates == 0)
                            {
                                await SaveWeight(connection1, o, validicObject, x, idActivity, idSource);
                            }
                        }
                        else if (validicObject == 5)
                        {
                            int updates = await UpdateDiabetes(connection1, o, validicObject, x, idActivity, idSource);
                            if (updates == 0)
                            {
                                await SaveDiabetes(connection1, o, validicObject, x, idActivity, idSource);
                            }
                        }
                        else if (validicObject == 6)
                        {
                            int updates = await UpdateBiometrics(connection1, o, validicObject, x, idActivity, idSource);
                            if (updates == 0)
                            {
                                await SaveBiometrics(connection1, o, validicObject, x, idActivity, idSource);
                            }
                        }

                    }

                    int updatesIds = await UserUpdate(connection1, validicObject, nowDate, next);
                    if (updatesIds == 0)
                    { 
                        await InsertUserUpdate(connection1, validicObject, nowDate, next);
                    }
                }
        ...
        async Task<int> UpdateFitness(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task SaveFitness(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task<int> UpdateRoutine(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task SaveRoutine(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task<int> UpdateNutrition(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task SaveNutrition(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task<int> UpdateSleep(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task SaveSleep(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task<int> UpdateWeight(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task SaveWeight(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task<int> UpdateDiabetes(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task SaveDiabetes(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
        ...
        async Task<int> UpdateBiometrics(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
            String strcommand = "update ValidicDiabetes " +
        ...
        async Task SaveBiometrics(SqlConnection connection1, JToken o, int validicObject, int x, string idActivity, string idSource)
        {
            String strcommand = "INSERT INTO ValidicDiabetes " +
        ...
        async Task<int> UserUpdate(SqlConnection connection1, int validicObject, DateTime nowDate, string next)
        {
            String strcommand = "update ValidicUpdate set ";
           
            if (validicObject == 6)
            {
                strcommand += "startdate = @lastupdate, ";
            ...
            SqlParameter parameter = new SqlParameter("@next", SqlDbType.VarChar);
            parameter.Value = next;
            command1.Parameters.Add(parameter);

            if (validicObject == 6)
            {
                parameter = new SqlParameter("@lastupdate", SqlDbType.DateTime);
        ...
        async Task InsertUserUpdate(SqlConnection connection1, int validicObject, DateTime nowDate, string next)
        {
            String strcommand = "insert into ValidicUpdate (startdate,";
            ...
            strcommand += "values(@lastupdate,@next)";

            SqlCommand command1 = new SqlCommand(strcommand, connection1);

            SqlParameter parameter = new SqlParameter("@lastupdate", SqlDbType.DateTime);
            parameter.Value = nowDate;
            command1.Parameters.Add(parameter);
