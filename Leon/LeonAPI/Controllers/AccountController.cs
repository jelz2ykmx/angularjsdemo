...
using LeonAPI.Providers;
using LeonAPI.Results;
using LeonModels.Catalogs;
using System.Linq;
using LeonModels;

namespace LeonAPI.Controllers
{
    [Authorize]
    ...
    public class AccountController : ApiController
    {
        ...
        [Authorize(Roles = "Administrador")]
        [Route("RegisterUser")]
        public async Task<IHttpActionResult> RegisterUser(RegisterUserModel model)
        {
            ...
            else
            {
                await Task.Run(() =>
                {
                    role = roleManager.Roles.Where(r => r.Name.Equals(model.role, StringComparison.CurrentCultureIgnoreCase)).FirstOrDefault();
                });
            }
            ...
            if (!result.Succeeded)
            {
                ...
                return ResponseMessage(Request.CreateResponse(System.Net.HttpStatusCode.InternalServerError, _errors));
            }
        ...
        public async Task<IHttpActionResult> ChangeUserPassword(ChangePasswordModel model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
